package com.appthings.app.data.local

import android.content.Context
import android.content.SharedPreferences
import com.appthings.app.data.model.ActionState
import com.appthings.app.data.model.AuthUser
import com.appthings.app.util.getObject
import com.appthings.app.util.putObject
...
class AuthPref(val context:Context) {
    ...
    suspend fun login(email: String, password: String): ActionState<AuthUser> {
        ...
        return if (user == null){
            ...
            ActionState(message = "Email dan Password kosong", isSuccess = false)
        } else if (user.email == email || user.password == password) {
            isLogin = true
            ActionState(authUser, message = "Anda berhasil login")
        } else {
            ActionState(message = "Email / password salah!", isSuccess = false)
        }
    }
